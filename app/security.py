...
def inspect_prompt(text: str):
    ...
    return {
        "risk": "high" if len(findings) > 1 else "medium" if findings else "low",
        "findings": findings,
    }
...
def redact(value):
    ...
    for key, val in os.environ.items():
        if (
            any(x in key.upper() for x in ("KEY", "TOKEN", "SECRET", "PASSWORD"))
            and len(val) > 7
        ):
...
def validate_argv(argv: list[str]):
    ...
    if binary in BLOCKED_BINARIES:
        raise ValueError(f"'{binary}' is blocked")
    if binary in DESTRUCTIVE:
        raise ValueError(
            f"Destructive command '{binary}' is not available in this proof of concept"
        )
