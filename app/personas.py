...
def load_directory(path: Path):
    ...
    for file in sorted(path.glob("*.md")):
        try:
            data = load_persona(file)
            raw[data.get("id", file.stem)] = data
    ...
    def resolve(ident, stack=()):
        ...
        if ident in stack:
            raise ValueError(
                f"Persona inheritance cycle: {' -> '.join(stack + (ident,))}"
            )
        if ident not in raw:
            raise ValueError(f"Unknown persona layer: {ident}")
        data = dict(raw[ident])
        layers = data.get("extends", [])
        layers = [layers] if isinstance(layers, str) else layers
        parents = [resolve(x, stack + (ident,)) for x in layers]
        data["persona_layers"] = layers
        data["persona"] = "\n\n".join(
            [x["persona"] for x in parents] + [data["persona"]]
        )
        resolved[ident] = data
        return data
