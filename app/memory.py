...
async def _embedding(text: str):
    async with httpx.AsyncClient(timeout=20) as c:
        r = await c.post(
            settings.ollama_url.rstrip("/") + "/api/embeddings",
            json={"model": settings.embedding_model, "prompt": text[:12000]},
        )
        r.raise_for_status()
...
async def remember(session_id, agent_id, role, content):
    try:
        ...
        point_id = int(
            hashlib.sha256(
                f"{session_id}:{agent_id}:{role}:{content}".encode()
            ).hexdigest()[:15],
            16,
        )
        async with httpx.AsyncClient(timeout=10) as c:
