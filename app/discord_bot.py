...
ALLOWED = {
    int(x)
    for x in os.getenv("AENIMUS_DISCORD_ALLOWED_USER_IDS", "").split(",")
    if x.strip()
}
...
async def request(method, path, **kwargs):
    async with httpx.AsyncClient(timeout=180) as c:
        r = await c.request(method, API + path, **kwargs)
        r.raise_for_status()
        return r.json()
...
async def on_message(message):
    if (
        message.author.bot
        or message.channel.id != CHANNEL
        or not message.content.startswith(PREFIX)
    ):
        return
    if ALLOWED and message.author.id not in ALLOWED:
        await message.reply("AENIMUS: sender is not authorized.")
        return
    prompt = message.content[len(PREFIX) :].strip()
    if not prompt:
        await message.reply(f"Usage: `{PREFIX} your instruction`")
        return
    sessions = await request("GET", "/sessions")
    if not sessions:
        await message.reply("No active AENIMUS operation exists.")
        return
    session = sessions[0]
    await message.add_reaction("⚙️")
    result = await request(
        "POST", f"/sessions/{session['id']}/chat", json={"content": prompt}
    )
    ...
    for start in range(0, len(final), 1900):
        await message.reply(final[start : start + 1900])
