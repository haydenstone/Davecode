...
class StorePersonaTests(unittest.TestCase):
    ...
    def test_online_backup_and_restore(self):
        with TemporaryDirectory() as tmp:
            root=Path(tmp); store=Store(root/"live.db")
            store.put_agent({"id":"agent-one"}); store.backup(root/"backup.db")
            store.put_agent({"id":"agent-two"}); store.restore(root/"backup.db")
    ...
    def test_approval_is_payload_bound_and_single_use(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertFalse(store.consume_approval(approval["id"],"terminal.exec",{"argv":["id"],"cwd":"."}))
            self.assertTrue(store.consume_approval(approval["id"],"terminal.exec",{"argv":["ls"],"cwd":"."}))
            self.assertFalse(store.consume_approval(approval["id"],"terminal.exec",{"argv":["ls"],"cwd":"."}))
